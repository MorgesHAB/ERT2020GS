...
void GSESensors::write(Packet &packet) {
    ...
    packet.write(hose_disconnect_state);
...
void GSESensors::parse(Packet &packet) {
    ...
    packet.parse(hose_disconnect_state);
...
void GSESensors::print() const {
    std::cout << "GSE SENSORS PACKET\n" << std::endl <<
              "States:  [" << fill_valve_state << "," << purge_valve_state << "," << main_ignition_state << ","
              << secondary_ignition_state << "," << hose_disconnect_state << "]\n"
...
bool GSESensors::updateRx(std::shared_ptr<Connector> connector){
    ...
    connector->setData(ui_interface::GSE_HOSE_DISCONNECT_STATE, hose_disconnect_state);
